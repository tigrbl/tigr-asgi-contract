BINDING_FAMILY_MATRIX = {
  "rest": {
    "request": "R",
    "session": "F",
    "message": "F",
    "stream": "O",
    "datagram": "F",
    "lifespan": "F"
  },
  "jsonrpc": {
    "request": "R",
    "session": "F",
    "message": "F",
    "stream": "O",
    "datagram": "F",
    "lifespan": "F"
  },
  "http.stream": {
    "request": "R",
    "session": "F",
    "message": "F",
    "stream": "R",
    "datagram": "F",
    "lifespan": "F"
  },
  "sse": {
    "request": "R",
    "session": "R",
    "message": "R",
    "stream": "R",
    "datagram": "F",
    "lifespan": "F"
  },
  "websocket": {
    "request": "F",
    "session": "R",
    "message": "R",
    "stream": "F",
    "datagram": "F",
    "lifespan": "F"
  },
  "webtransport": {
    "request": "F",
    "session": "R",
    "message": "F",
    "stream": "R",
    "datagram": "R",
    "lifespan": "F"
  },
  "lifespan": {
    "request": "F",
    "session": "F",
    "message": "F",
    "stream": "F",
    "datagram": "F",
    "lifespan": "R"
  }
}

FAMILY_SUBEVENT_MATRIX = {
  "request": {
    "request.open": "R",
    "request.body_in": "R",
    "request.chunk_in": "R",
    "request.dispatch": "R",
    "request.close": "R",
    "request.disconnect": "R",
    "response.open": "R",
    "response.body_out": "R",
    "response.chunk_out": "R",
    "response.finalize": "R",
    "response.emit_complete": "R"
  },
  "session": {
    "session.open": "R",
    "session.accept": "R",
    "session.reject": "R",
    "session.ready": "R",
    "session.heartbeat": "R",
    "session.sync": "R",
    "session.close": "R",
    "session.disconnect": "R",
    "session.emit_complete": "R"
  },
  "message": {
    "message.in": "R",
    "message.decode": "R",
    "message.decode_failed": "R",
    "message.handle": "R",
    "message.handle_failed": "R",
    "message.out": "R",
    "message.replay": "R",
    "message.snapshot": "R",
    "message.emit_complete": "R",
    "message.emit_failed": "R"
  },
  "stream": {
    "stream.open": "R",
    "stream.chunk_in": "R",
    "stream.chunk_out": "R",
    "stream.flush": "R",
    "stream.finalize": "R",
    "stream.reset": "R",
    "stream.stop_sending": "R",
    "stream.close": "R",
    "stream.emit_complete": "R"
  },
  "datagram": {
    "datagram.in": "R",
    "datagram.handle": "R",
    "datagram.out": "R",
    "datagram.emit_complete": "R",
    "datagram.emit_failed": "R"
  },
  "lifespan": {
    "lifespan.startup": "R",
    "lifespan.startup_complete": "R",
    "lifespan.startup_failed": "R",
    "lifespan.shutdown": "R",
    "lifespan.shutdown_complete": "R",
    "lifespan.shutdown_failed": "R"
  }
}

BINDING_SUBEVENT_MATRIX = {
  "rest": {
    "request.open": "R",
    "request.body_in": "R",
    "request.chunk_in": "F",
    "request.dispatch": "D",
    "request.close": "R",
    "request.disconnect": "O",
    "response.open": "R",
    "response.body_out": "R",
    "response.chunk_out": "F",
    "response.finalize": "O",
    "response.emit_complete": "R",
    "session.open": "F",
    "session.accept": "F",
    "session.reject": "F",
    "session.ready": "F",
    "session.heartbeat": "F",
    "session.sync": "F",
    "session.close": "F",
    "session.disconnect": "F",
    "session.emit_complete": "F",
    "message.in": "F",
    "message.decode": "F",
    "message.decode_failed": "F",
    "message.handle": "F",
    "message.handle_failed": "F",
    "message.out": "F",
    "message.replay": "F",
    "message.snapshot": "F",
    "message.emit_complete": "F",
    "message.emit_failed": "F",
    "stream.open": "O",
    "stream.chunk_in": "O",
    "stream.chunk_out": "O",
    "stream.flush": "F",
    "stream.finalize": "O",
    "stream.reset": "F",
    "stream.stop_sending": "F",
    "stream.close": "O",
    "stream.emit_complete": "O",
    "datagram.in": "F",
    "datagram.handle": "F",
    "datagram.out": "F",
    "datagram.emit_complete": "F",
    "datagram.emit_failed": "F",
    "lifespan.startup": "F",
    "lifespan.startup_complete": "F",
    "lifespan.startup_failed": "F",
    "lifespan.shutdown": "F",
    "lifespan.shutdown_complete": "F",
    "lifespan.shutdown_failed": "F"
  },
  "jsonrpc": {
    "request.open": "R",
    "request.body_in": "R",
    "request.chunk_in": "F",
    "request.dispatch": "D",
    "request.close": "R",
    "request.disconnect": "O",
    "response.open": "R",
    "response.body_out": "R",
    "response.chunk_out": "F",
    "response.finalize": "O",
    "response.emit_complete": "R",
    "session.open": "F",
    "session.accept": "F",
    "session.reject": "F",
    "session.ready": "F",
    "session.heartbeat": "F",
    "session.sync": "F",
    "session.close": "F",
    "session.disconnect": "F",
    "session.emit_complete": "F",
    "message.in": "F",
    "message.decode": "D",
    "message.decode_failed": "D",
    "message.handle": "F",
    "message.handle_failed": "F",
    "message.out": "F",
    "message.replay": "F",
    "message.snapshot": "F",
    "message.emit_complete": "F",
    "message.emit_failed": "F",
    "stream.open": "O",
    "stream.chunk_in": "O",
    "stream.chunk_out": "O",
    "stream.flush": "F",
    "stream.finalize": "O",
    "stream.reset": "F",
    "stream.stop_sending": "F",
    "stream.close": "O",
    "stream.emit_complete": "O",
    "datagram.in": "F",
    "datagram.handle": "F",
    "datagram.out": "F",
    "datagram.emit_complete": "F",
    "datagram.emit_failed": "F",
    "lifespan.startup": "F",
    "lifespan.startup_complete": "F",
    "lifespan.startup_failed": "F",
    "lifespan.shutdown": "F",
    "lifespan.shutdown_complete": "F",
    "lifespan.shutdown_failed": "F"
  },
  "http.stream": {
    "request.open": "R",
    "request.body_in": "O",
    "request.chunk_in": "D",
    "request.dispatch": "D",
    "request.close": "R",
    "request.disconnect": "O",
    "response.open": "R",
    "response.body_out": "O",
    "response.chunk_out": "D",
    "response.finalize": "O",
    "response.emit_complete": "R",
    "session.open": "F",
    "session.accept": "F",
    "session.reject": "F",
    "session.ready": "F",
    "session.heartbeat": "F",
    "session.sync": "F",
    "session.close": "F",
    "session.disconnect": "F",
    "session.emit_complete": "F",
    "message.in": "F",
    "message.decode": "F",
    "message.decode_failed": "F",
    "message.handle": "F",
    "message.handle_failed": "F",
    "message.out": "F",
    "message.replay": "F",
    "message.snapshot": "F",
    "message.emit_complete": "F",
    "message.emit_failed": "F",
    "stream.open": "R",
    "stream.chunk_in": "R",
    "stream.chunk_out": "R",
    "stream.flush": "O",
    "stream.finalize": "R",
    "stream.reset": "O",
    "stream.stop_sending": "F",
    "stream.close": "R",
    "stream.emit_complete": "R",
    "datagram.in": "F",
    "datagram.handle": "F",
    "datagram.out": "F",
    "datagram.emit_complete": "F",
    "datagram.emit_failed": "F",
    "lifespan.startup": "F",
    "lifespan.startup_complete": "F",
    "lifespan.startup_failed": "F",
    "lifespan.shutdown": "F",
    "lifespan.shutdown_complete": "F",
    "lifespan.shutdown_failed": "F"
  },
  "sse": {
    "request.open": "R",
    "request.body_in": "O",
    "request.chunk_in": "F",
    "request.dispatch": "D",
    "request.close": "R",
    "request.disconnect": "O",
    "response.open": "O",
    "response.body_out": "F",
    "response.chunk_out": "D",
    "response.finalize": "O",
    "response.emit_complete": "R",
    "session.open": "R",
    "session.accept": "R",
    "session.reject": "F",
    "session.ready": "R",
    "session.heartbeat": "O",
    "session.sync": "O",
    "session.close": "R",
    "session.disconnect": "O",
    "session.emit_complete": "O",
    "message.in": "F",
    "message.decode": "F",
    "message.decode_failed": "F",
    "message.handle": "F",
    "message.handle_failed": "F",
    "message.out": "R",
    "message.replay": "O",
    "message.snapshot": "O",
    "message.emit_complete": "R",
    "message.emit_failed": "O",
    "stream.open": "R",
    "stream.chunk_in": "F",
    "stream.chunk_out": "R",
    "stream.flush": "O",
    "stream.finalize": "O",
    "stream.reset": "O",
    "stream.stop_sending": "F",
    "stream.close": "R",
    "stream.emit_complete": "R",
    "datagram.in": "F",
    "datagram.handle": "F",
    "datagram.out": "F",
    "datagram.emit_complete": "F",
    "datagram.emit_failed": "F",
    "lifespan.startup": "F",
    "lifespan.startup_complete": "F",
    "lifespan.startup_failed": "F",
    "lifespan.shutdown": "F",
    "lifespan.shutdown_complete": "F",
    "lifespan.shutdown_failed": "F"
  },
  "websocket": {
    "request.open": "F",
    "request.body_in": "F",
    "request.chunk_in": "F",
    "request.dispatch": "F",
    "request.close": "F",
    "request.disconnect": "F",
    "response.open": "F",
    "response.body_out": "F",
    "response.chunk_out": "F",
    "response.finalize": "F",
    "response.emit_complete": "F",
    "session.open": "R",
    "session.accept": "R",
    "session.reject": "O",
    "session.ready": "R",
    "session.heartbeat": "O",
    "session.sync": "O",
    "session.close": "R",
    "session.disconnect": "O",
    "session.emit_complete": "O",
    "message.in": "R",
    "message.decode": "O",
    "message.decode_failed": "O",
    "message.handle": "R",
    "message.handle_failed": "O",
    "message.out": "R",
    "message.replay": "O",
    "message.snapshot": "O",
    "message.emit_complete": "R",
    "message.emit_failed": "O",
    "stream.open": "F",
    "stream.chunk_in": "F",
    "stream.chunk_out": "F",
    "stream.flush": "F",
    "stream.finalize": "F",
    "stream.reset": "F",
    "stream.stop_sending": "F",
    "stream.close": "F",
    "stream.emit_complete": "F",
    "datagram.in": "F",
    "datagram.handle": "F",
    "datagram.out": "F",
    "datagram.emit_complete": "F",
    "datagram.emit_failed": "F",
    "lifespan.startup": "F",
    "lifespan.startup_complete": "F",
    "lifespan.startup_failed": "F",
    "lifespan.shutdown": "F",
    "lifespan.shutdown_complete": "F",
    "lifespan.shutdown_failed": "F"
  },
  "webtransport": {
    "request.open": "F",
    "request.body_in": "F",
    "request.chunk_in": "F",
    "request.dispatch": "F",
    "request.close": "F",
    "request.disconnect": "F",
    "response.open": "F",
    "response.body_out": "F",
    "response.chunk_out": "F",
    "response.finalize": "F",
    "response.emit_complete": "F",
    "session.open": "R",
    "session.accept": "R",
    "session.reject": "O",
    "session.ready": "R",
    "session.heartbeat": "O",
    "session.sync": "O",
    "session.close": "R",
    "session.disconnect": "O",
    "session.emit_complete": "O",
    "message.in": "F",
    "message.decode": "F",
    "message.decode_failed": "F",
    "message.handle": "F",
    "message.handle_failed": "F",
    "message.out": "F",
    "message.replay": "F",
    "message.snapshot": "F",
    "message.emit_complete": "F",
    "message.emit_failed": "F",
    "stream.open": "R",
    "stream.chunk_in": "R",
    "stream.chunk_out": "R",
    "stream.flush": "O",
    "stream.finalize": "R",
    "stream.reset": "O",
    "stream.stop_sending": "O",
    "stream.close": "R",
    "stream.emit_complete": "R",
    "datagram.in": "R",
    "datagram.handle": "R",
    "datagram.out": "R",
    "datagram.emit_complete": "R",
    "datagram.emit_failed": "O",
    "lifespan.startup": "F",
    "lifespan.startup_complete": "F",
    "lifespan.startup_failed": "F",
    "lifespan.shutdown": "F",
    "lifespan.shutdown_complete": "F",
    "lifespan.shutdown_failed": "F"
  },
  "lifespan": {
    "request.open": "F",
    "request.body_in": "F",
    "request.chunk_in": "F",
    "request.dispatch": "F",
    "request.close": "F",
    "request.disconnect": "F",
    "response.open": "F",
    "response.body_out": "F",
    "response.chunk_out": "F",
    "response.finalize": "F",
    "response.emit_complete": "F",
    "session.open": "F",
    "session.accept": "F",
    "session.reject": "F",
    "session.ready": "F",
    "session.heartbeat": "F",
    "session.sync": "F",
    "session.close": "F",
    "session.disconnect": "F",
    "session.emit_complete": "F",
    "message.in": "F",
    "message.decode": "F",
    "message.decode_failed": "F",
    "message.handle": "F",
    "message.handle_failed": "F",
    "message.out": "F",
    "message.replay": "F",
    "message.snapshot": "F",
    "message.emit_complete": "F",
    "message.emit_failed": "F",
    "stream.open": "F",
    "stream.chunk_in": "F",
    "stream.chunk_out": "F",
    "stream.flush": "F",
    "stream.finalize": "F",
    "stream.reset": "F",
    "stream.stop_sending": "F",
    "stream.close": "F",
    "stream.emit_complete": "F",
    "datagram.in": "F",
    "datagram.handle": "F",
    "datagram.out": "F",
    "datagram.emit_complete": "F",
    "datagram.emit_failed": "F",
    "lifespan.startup": "R",
    "lifespan.startup_complete": "R",
    "lifespan.startup_failed": "O",
    "lifespan.shutdown": "R",
    "lifespan.shutdown_complete": "R",
    "lifespan.shutdown_failed": "O"
  }
}

PROTOCOLS = {'asgi.pathsend': {'binding': 'http.stream',
                   'scope_type': 'http',
                   'secure': False,
                   'transport': 'http'},
 'http.jsonrpc': {'binding': 'jsonrpc',
                  'scope_type': 'http',
                  'secure': False,
                  'transport': 'http'},
 'http.rest': {'binding': 'rest',
               'scope_type': 'http',
               'secure': False,
               'transport': 'http'},
 'http.sse': {'binding': 'sse',
              'scope_type': 'http',
              'secure': False,
              'transport': 'http'},
 'http.stream': {'binding': 'http.stream',
                 'scope_type': 'http',
                 'secure': False,
                 'transport': 'http'},
 'https.jsonrpc': {'binding': 'jsonrpc',
                   'scope_type': 'http',
                   'secure': True,
                   'transport': 'http'},
 'https.rest': {'binding': 'rest',
                'scope_type': 'http',
                'secure': True,
                'transport': 'http'},
 'https.sse': {'binding': 'sse',
               'scope_type': 'http',
               'secure': True,
               'transport': 'http'},
 'https.stream': {'binding': 'http.stream',
                  'scope_type': 'http',
                  'secure': True,
                  'transport': 'http'},
 'lifespan': {'binding': 'lifespan',
              'scope_type': 'lifespan',
              'secure': False,
              'transport': 'lifespan'},
 'webtransport': {'binding': 'webtransport',
                  'scope_type': 'webtransport',
                  'secure': True,
                  'transport': 'webtransport'},
 'ws': {'binding': 'websocket',
        'scope_type': 'websocket',
        'secure': False,
        'transport': 'websocket'},
 'wss': {'binding': 'websocket',
         'scope_type': 'websocket',
         'secure': True,
         'transport': 'websocket'}}

AUTOMATA = {'datagram': {'initial': 'idle',
              'terminal': ['complete', 'failed'],
              'transitions': [{'event': 'datagram.in',
                               'from': 'idle',
                               'to': 'received'},
                              {'event': 'datagram.handle',
                               'from': 'received',
                               'to': 'handled'},
                              {'event': 'datagram.out',
                               'from': 'handled',
                               'to': 'emitted'},
                              {'event': 'datagram.emit_complete',
                               'from': 'emitted',
                               'to': 'complete'},
                              {'event': 'datagram.emit_failed',
                               'from': 'emitted',
                               'to': 'failed'}]},
 'lifespan': {'initial': 'idle',
              'terminal': ['startup_complete',
                           'startup_failed',
                           'shutdown_complete',
                           'shutdown_failed'],
              'transitions': [{'event': 'lifespan.startup',
                               'from': 'idle',
                               'to': 'starting'},
                              {'event': 'lifespan.startup_complete',
                               'from': 'starting',
                               'to': 'startup_complete'},
                              {'event': 'lifespan.startup_failed',
                               'from': 'starting',
                               'to': 'startup_failed'},
                              {'event': 'lifespan.shutdown',
                               'from': 'startup_complete',
                               'to': 'shutting_down'},
                              {'event': 'lifespan.shutdown',
                               'from': 'idle',
                               'to': 'shutting_down'},
                              {'event': 'lifespan.shutdown_complete',
                               'from': 'shutting_down',
                               'to': 'shutdown_complete'},
                              {'event': 'lifespan.shutdown_failed',
                               'from': 'shutting_down',
                               'to': 'shutdown_failed'}]},
 'message': {'initial': 'idle',
             'terminal': ['complete', 'failed'],
             'transitions': [{'event': 'message.in',
                              'from': 'idle',
                              'to': 'received'},
                             {'event': 'message.decode',
                              'from': 'received',
                              'to': 'decoded'},
                             {'event': 'message.decode_failed',
                              'from': 'received',
                              'to': 'failed'},
                             {'event': 'message.handle',
                              'from': 'decoded',
                              'to': 'handled'},
                             {'event': 'message.handle_failed',
                              'from': 'decoded',
                              'to': 'failed'},
                             {'event': 'message.out',
                              'from': 'handled',
                              'to': 'emitted'},
                             {'event': 'message.emit_complete',
                              'from': 'emitted',
                              'to': 'complete'},
                             {'event': 'message.emit_failed',
                              'from': 'emitted',
                              'to': 'failed'},
                             {'event': 'message.replay',
                              'from': 'complete',
                              'to': 'emitted'},
                             {'event': 'message.snapshot',
                              'from': 'complete',
                              'to': 'complete'}]},
 'request': {'initial': 'idle',
             'terminal': ['closed', 'disconnected'],
             'transitions': [{'event': 'request.open',
                              'from': 'idle',
                              'to': 'open'},
                             {'event': 'request.body_in',
                              'from': 'open',
                              'to': 'open'},
                             {'event': 'request.chunk_in',
                              'from': 'open',
                              'to': 'open'},
                             {'event': 'request.close',
                              'from': 'open',
                              'to': 'body_closed'},
                             {'event': 'request.dispatch',
                              'from': 'open',
                              'to': 'dispatched'},
                             {'event': 'request.dispatch',
                              'from': 'body_closed',
                              'to': 'dispatched'},
                             {'event': 'response.open',
                              'from': 'dispatched',
                              'to': 'responding'},
                             {'event': 'response.body_out',
                              'from': 'responding',
                              'to': 'responding'},
                             {'event': 'response.chunk_out',
                              'from': 'responding',
                              'to': 'responding'},
                             {'event': 'response.emit_complete',
                              'from': 'responding',
                              'to': 'completing'},
                             {'event': 'response.finalize',
                              'from': 'completing',
                              'to': 'closed'},
                             {'event': 'request.disconnect',
                              'from': 'open',
                              'to': 'disconnected'},
                             {'event': 'request.disconnect',
                              'from': 'body_closed',
                              'to': 'disconnected'}]},
 'session': {'initial': 'idle',
             'terminal': ['closed', 'disconnected', 'rejected'],
             'transitions': [{'event': 'session.open',
                              'from': 'idle',
                              'to': 'open'},
                             {'event': 'session.accept',
                              'from': 'open',
                              'to': 'accepted'},
                             {'event': 'session.reject',
                              'from': 'open',
                              'to': 'rejected'},
                             {'event': 'session.disconnect',
                              'from': 'open',
                              'to': 'disconnected'},
                             {'event': 'session.ready',
                              'from': 'accepted',
                              'to': 'ready'},
                             {'event': 'session.disconnect',
                              'from': 'accepted',
                              'to': 'disconnected'},
                             {'event': 'session.heartbeat',
                              'from': 'ready',
                              'to': 'ready'},
                             {'event': 'session.sync',
                              'from': 'ready',
                              'to': 'ready'},
                             {'event': 'session.emit_complete',
                              'from': 'ready',
                              'to': 'ready'},
                             {'event': 'session.close',
                              'from': 'ready',
                              'to': 'closed'},
                             {'event': 'session.disconnect',
                              'from': 'ready',
                              'to': 'disconnected'}]},
 'stream': {'initial': 'idle',
            'terminal': ['closed', 'reset', 'stopped'],
            'transitions': [{'event': 'stream.open',
                             'from': 'idle',
                             'to': 'open'},
                            {'event': 'stream.chunk_in',
                             'from': 'open',
                             'to': 'open'},
                            {'event': 'stream.chunk_out',
                             'from': 'open',
                             'to': 'open'},
                            {'event': 'stream.flush',
                             'from': 'open',
                             'to': 'open'},
                            {'event': 'stream.finalize',
                             'from': 'open',
                             'to': 'finalizing'},
                            {'event': 'stream.emit_complete',
                             'from': 'finalizing',
                             'to': 'closed'},
                            {'event': 'stream.close',
                             'from': 'open',
                             'to': 'closed'},
                            {'event': 'stream.reset',
                             'from': 'open',
                             'to': 'reset'},
                            {'event': 'stream.stop_sending',
                             'from': 'open',
                             'to': 'stopped'},
                            {'event': 'stream.reset',
                             'from': 'finalizing',
                             'to': 'reset'},
                            {'event': 'stream.stop_sending',
                             'from': 'finalizing',
                             'to': 'stopped'}]}}

FRAMES = {'app': {'binding': 'webtransport',
         'kind': 'contract-framing',
         'schema': 'contract/schemas/frames/app.schema.json'},
 'asgi-pathsend-extension': {'binding': 'http.stream',
                             'kind': 'extension-frame',
                             'schema': 'contract/schemas/frames/asgi-pathsend-extension.schema.json'},
 'asgi-tls-extension': {'binding': '*',
                        'kind': 'extension-frame',
                        'schema': 'contract/schemas/frames/asgi-tls-extension.schema.json'},
 'bytes': {'binding': 'http.stream',
           'kind': 'contract-framing',
           'schema': 'contract/schemas/frames/bytes.schema.json'},
 'grpc': {'binding': 'http.stream',
          'kind': 'contract-framing',
          'schema': 'contract/schemas/frames/grpc.schema.json'},
 'http-1-1-message': {'binding': 'rest',
                      'kind': 'wire-frame',
                      'schema': 'contract/schemas/frames/http-1-1-message.schema.json'},
 'http-request-body-chunk': {'binding': 'http.stream',
                             'kind': 'wire-frame',
                             'schema': 'contract/schemas/frames/http-request-body-chunk.schema.json'},
 'http-response-body-chunk': {'binding': 'http.stream',
                              'kind': 'wire-frame',
                              'schema': 'contract/schemas/frames/http-response-body-chunk.schema.json'},
 'http-response-start-frame': {'binding': 'rest',
                               'kind': 'wire-frame',
                               'schema': 'contract/schemas/frames/http-response-start-frame.schema.json'},
 'json': {'binding': 'rest',
          'kind': 'contract-framing',
          'schema': 'contract/schemas/frames/json.schema.json'},
 'json-rpc-error-object': {'binding': 'jsonrpc',
                           'kind': 'contract-framing',
                           'schema': 'contract/schemas/frames/json-rpc-error-object.schema.json'},
 'json-rpc-notification-object': {'binding': 'jsonrpc',
                                  'kind': 'contract-framing',
                                  'schema': 'contract/schemas/frames/json-rpc-notification-object.schema.json'},
 'json-rpc-request-object': {'binding': 'jsonrpc',
                             'kind': 'contract-framing',
                             'schema': 'contract/schemas/frames/json-rpc-request-object.schema.json'},
 'json-rpc-response-object': {'binding': 'jsonrpc',
                              'kind': 'contract-framing',
                              'schema': 'contract/schemas/frames/json-rpc-response-object.schema.json'},
 'jsonrpc': {'binding': 'jsonrpc',
             'kind': 'contract-framing',
             'schema': 'contract/schemas/frames/jsonrpc.schema.json'},
 'raw': {'binding': 'websocket',
         'kind': 'contract-framing',
         'schema': 'contract/schemas/frames/raw.schema.json'},
 'sse': {'binding': 'sse',
         'kind': 'contract-framing',
         'schema': 'contract/schemas/frames/sse.schema.json'},
 'sse-data-field': {'binding': 'sse',
                    'kind': 'contract-framing',
                    'schema': 'contract/schemas/frames/sse-data-field.schema.json'},
 'sse-event-field': {'binding': 'sse',
                     'kind': 'contract-framing',
                     'schema': 'contract/schemas/frames/sse-event-field.schema.json'},
 'sse-id-field': {'binding': 'sse',
                  'kind': 'contract-framing',
                  'schema': 'contract/schemas/frames/sse-id-field.schema.json'},
 'sse-retry-field': {'binding': 'sse',
                     'kind': 'contract-framing',
                     'schema': 'contract/schemas/frames/sse-retry-field.schema.json'},
 'websocket-accept-frame': {'binding': 'websocket',
                            'kind': 'wire-frame',
                            'schema': 'contract/schemas/frames/websocket-accept-frame.schema.json'},
 'websocket-close-frame': {'binding': 'websocket',
                           'kind': 'wire-frame',
                           'schema': 'contract/schemas/frames/websocket-close-frame.schema.json'},
 'websocket-continuation-frame': {'binding': 'websocket',
                                  'kind': 'wire-frame',
                                  'schema': 'contract/schemas/frames/websocket-continuation-frame.schema.json'},
 'websocket-disconnect-frame': {'binding': 'websocket',
                                'kind': 'wire-frame',
                                'schema': 'contract/schemas/frames/websocket-disconnect-frame.schema.json'},
 'websocket-frame': {'binding': 'websocket',
                     'kind': 'wire-frame',
                     'schema': 'contract/schemas/frames/websocket-frame.schema.json'},
 'websocket-ping-frame': {'binding': 'websocket',
                          'kind': 'wire-frame',
                          'schema': 'contract/schemas/frames/websocket-ping-frame.schema.json'},
 'websocket-pong-frame': {'binding': 'websocket',
                          'kind': 'wire-frame',
                          'schema': 'contract/schemas/frames/websocket-pong-frame.schema.json'},
 'websocket-receive-bytes': {'binding': 'websocket',
                             'kind': 'wire-frame',
                             'schema': 'contract/schemas/frames/websocket-receive-bytes.schema.json'},
 'websocket-receive-text': {'binding': 'websocket',
                            'kind': 'wire-frame',
                            'schema': 'contract/schemas/frames/websocket-receive-text.schema.json'},
 'websocket-send-bytes': {'binding': 'websocket',
                          'kind': 'wire-frame',
                          'schema': 'contract/schemas/frames/websocket-send-bytes.schema.json'},
 'websocket-send-text': {'binding': 'websocket',
                         'kind': 'wire-frame',
                         'schema': 'contract/schemas/frames/websocket-send-text.schema.json'},
 'webtransport-datagram-frame': {'binding': 'webtransport',
                                 'kind': 'wire-frame',
                                 'schema': 'contract/schemas/frames/webtransport-datagram-frame.schema.json'},
 'webtransport-stream-frame': {'binding': 'webtransport',
                               'kind': 'wire-frame',
                               'schema': 'contract/schemas/frames/webtransport-stream-frame.schema.json'}}

EXTENSIONS = {'asgi.pathsend': {'event_type': 'http.response.pathsend',
                   'fields': {'headers': {'required': False, 'type': 'array'},
                              'path': {'required': True, 'type': 'string'},
                              'stat_result': {'required': False,
                                              'type': 'object'}}},
 'asgi.tls': {'fields': {'cipher': {'required': False, 'type': 'string'},
                         'peer_cert': {'required': False, 'type': 'object'},
                         'peer_identity': {'required': False, 'type': 'string'},
                         'verified': {'required': False, 'type': 'boolean'},
                         'version': {'required': False, 'type': 'string'}},
              'scope_key': 'tls'}}

CHANNELS = ['receive', 'send']

DIRECTIONS = ['client_to_server',
 'server_to_client',
 'app_to_server',
 'server_to_app',
 'system']

FRAMINGS = ['json', 'jsonrpc', 'ndjson', 'sse', 'text', 'bytes', 'binary']

EVENT_CLASSIFICATIONS = [{'allowed_framings': ['json', 'text', 'bytes', 'binary'],
  'binding': 'rest',
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'http.request',
  'exchange': 'unary',
  'family': 'request',
  'required_payload_fields': [],
  'scope_type': 'http'},
 {'allowed_framings': ['jsonrpc'],
  'binding': 'jsonrpc',
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'http.request',
  'exchange': 'unary',
  'family': 'request',
  'required_payload_fields': [],
  'scope_type': 'http'},
 {'allowed_framings': ['json', 'ndjson', 'text', 'bytes', 'binary'],
  'binding': 'http.stream',
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'http.request',
  'exchange': 'client_stream',
  'family': 'stream',
  'required_payload_fields': [],
  'scope_type': 'http'},
 {'allowed_framings': [],
  'binding': 'rest',
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'http.disconnect',
  'exchange': 'unary',
  'family': 'request',
  'required_payload_fields': [],
  'scope_type': 'http'},
 {'allowed_framings': [],
  'binding': 'rest',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'http.response.start',
  'exchange': 'unary',
  'family': 'request',
  'required_payload_fields': [],
  'scope_type': 'http'},
 {'allowed_framings': [],
  'binding': 'jsonrpc',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'http.response.start',
  'exchange': 'unary',
  'family': 'request',
  'required_payload_fields': [],
  'scope_type': 'http'},
 {'allowed_framings': ['json', 'text', 'bytes', 'binary'],
  'binding': 'rest',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'http.response.body',
  'exchange': 'unary',
  'family': 'request',
  'required_payload_fields': [],
  'scope_type': 'http'},
 {'allowed_framings': ['jsonrpc'],
  'binding': 'jsonrpc',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'http.response.body',
  'exchange': 'unary',
  'family': 'request',
  'required_payload_fields': [],
  'scope_type': 'http'},
 {'allowed_framings': ['json', 'ndjson', 'text', 'bytes', 'binary'],
  'binding': 'http.stream',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'http.response.body',
  'exchange': 'server_stream',
  'family': 'stream',
  'required_payload_fields': [],
  'scope_type': 'http'},
 {'allowed_framings': ['sse'],
  'binding': 'sse',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'http.response.body',
  'exchange': 'server_stream',
  'family': 'stream',
  'required_payload_fields': [],
  'scope_type': 'http'},
 {'allowed_framings': [],
  'binding': 'http.stream',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'http.response.pathsend',
  'exchange': 'unary',
  'family': 'request',
  'required_payload_fields': ['path'],
  'scope_type': 'http'},
 {'allowed_framings': [],
  'binding': 'websocket',
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'websocket.connect',
  'exchange': 'unary',
  'family': 'session',
  'required_payload_fields': [],
  'scope_type': 'websocket'},
 {'allowed_framings': ['json', 'jsonrpc', 'ndjson', 'text', 'bytes', 'binary'],
  'binding': 'websocket',
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'websocket.receive',
  'exchange': 'duplex',
  'family': 'message',
  'required_payload_fields': [],
  'scope_type': 'websocket'},
 {'allowed_framings': [],
  'binding': 'websocket',
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'websocket.disconnect',
  'exchange': 'unary',
  'family': 'session',
  'required_payload_fields': [],
  'scope_type': 'websocket'},
 {'allowed_framings': [],
  'binding': 'websocket',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'websocket.accept',
  'exchange': 'unary',
  'family': 'session',
  'required_payload_fields': [],
  'scope_type': 'websocket'},
 {'allowed_framings': ['json', 'jsonrpc', 'ndjson', 'text', 'bytes', 'binary'],
  'binding': 'websocket',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'websocket.send',
  'exchange': 'duplex',
  'family': 'message',
  'required_payload_fields': [],
  'scope_type': 'websocket'},
 {'allowed_framings': [],
  'binding': 'websocket',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'websocket.close',
  'exchange': 'unary',
  'family': 'session',
  'required_payload_fields': [],
  'scope_type': 'websocket'},
 {'allowed_framings': [],
  'binding': 'webtransport',
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'webtransport.connect',
  'exchange': 'unary',
  'family': 'session',
  'required_payload_fields': [],
  'scope_type': 'webtransport'},
 {'allowed_framings': [],
  'binding': 'webtransport',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'webtransport.accept',
  'exchange': 'unary',
  'family': 'session',
  'required_payload_fields': [],
  'scope_type': 'webtransport'},
 {'allowed_framings': ['json', 'jsonrpc', 'ndjson', 'text', 'bytes', 'binary'],
  'binding': 'webtransport',
  'capability_gates': ['supports_bidi_streams'],
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'webtransport.stream.receive',
  'exchange': 'duplex',
  'family': 'stream',
  'required_payload_fields': ['stream_id', 'stream_direction'],
  'scope_type': 'webtransport',
  'stream_direction': 'bidi'},
 {'allowed_framings': ['json', 'jsonrpc', 'ndjson', 'text', 'bytes', 'binary'],
  'binding': 'webtransport',
  'capability_gates': ['supports_uni_streams'],
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'webtransport.stream.receive',
  'exchange': 'client_stream',
  'family': 'stream',
  'required_payload_fields': ['stream_id', 'stream_direction'],
  'scope_type': 'webtransport',
  'stream_direction': 'client_to_server'},
 {'allowed_framings': ['json', 'jsonrpc', 'ndjson', 'text', 'bytes', 'binary'],
  'binding': 'webtransport',
  'capability_gates': ['supports_bidi_streams'],
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'webtransport.stream.send',
  'exchange': 'duplex',
  'family': 'stream',
  'required_payload_fields': ['stream_id', 'stream_direction'],
  'scope_type': 'webtransport',
  'stream_direction': 'bidi'},
 {'allowed_framings': ['json', 'ndjson', 'text', 'bytes', 'binary'],
  'binding': 'webtransport',
  'capability_gates': ['supports_uni_streams'],
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'webtransport.stream.send',
  'exchange': 'server_stream',
  'family': 'stream',
  'required_payload_fields': ['stream_id', 'stream_direction'],
  'scope_type': 'webtransport',
  'stream_direction': 'server_to_client'},
 {'allowed_framings': [],
  'binding': 'webtransport',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'webtransport.stream.close',
  'exchange': 'duplex',
  'family': 'stream',
  'required_payload_fields': ['stream_id'],
  'scope_type': 'webtransport'},
 {'allowed_framings': [],
  'binding': 'webtransport',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'webtransport.stream.reset',
  'exchange': 'duplex',
  'family': 'stream',
  'required_payload_fields': ['stream_id'],
  'scope_type': 'webtransport'},
 {'allowed_framings': [],
  'binding': 'webtransport',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'webtransport.stream.stop_sending',
  'exchange': 'duplex',
  'family': 'stream',
  'required_payload_fields': ['stream_id'],
  'scope_type': 'webtransport'},
 {'allowed_framings': [],
  'binding': 'webtransport',
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'stream.resume.request',
  'exchange': 'duplex',
  'family': 'stream',
  'required_payload_fields': ['resume_token',
                              'client_id',
                              'session_id',
                              'stream_id',
                              'binding'],
  'scope_type': 'webtransport'},
 {'allowed_framings': [],
  'binding': 'webtransport',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'stream.resume.accept',
  'exchange': 'duplex',
  'family': 'stream',
  'required_payload_fields': ['resume_token',
                              'client_id',
                              'session_id',
                              'stream_id',
                              'binding',
                              'accepted_offset'],
  'scope_type': 'webtransport'},
 {'allowed_framings': [],
  'binding': 'webtransport',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'stream.resume.reject',
  'exchange': 'duplex',
  'family': 'stream',
  'required_payload_fields': ['resume_token',
                              'client_id',
                              'session_id',
                              'stream_id',
                              'binding',
                              'reason'],
  'scope_type': 'webtransport'},
 {'allowed_framings': ['json', 'text', 'bytes', 'binary'],
  'binding': 'webtransport',
  'capability_gates': ['supports_datagrams'],
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'webtransport.datagram.receive',
  'exchange': 'duplex',
  'family': 'datagram',
  'required_payload_fields': ['datagram_id'],
  'scope_type': 'webtransport'},
 {'allowed_framings': ['json', 'text', 'bytes', 'binary'],
  'binding': 'webtransport',
  'capability_gates': ['supports_datagrams'],
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'webtransport.datagram.send',
  'exchange': 'duplex',
  'family': 'datagram',
  'required_payload_fields': ['datagram_id'],
  'scope_type': 'webtransport'},
 {'allowed_framings': [],
  'binding': 'webtransport',
  'channel': 'receive',
  'direction': 'client_to_server',
  'event': 'webtransport.disconnect',
  'exchange': 'unary',
  'family': 'session',
  'required_payload_fields': [],
  'scope_type': 'webtransport'},
 {'allowed_framings': [],
  'binding': 'webtransport',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'webtransport.close',
  'exchange': 'unary',
  'family': 'session',
  'required_payload_fields': [],
  'scope_type': 'webtransport'},
 {'allowed_framings': [],
  'binding': 'lifespan',
  'channel': 'receive',
  'direction': 'system',
  'event': 'lifespan.startup',
  'exchange': 'unary',
  'family': 'lifespan',
  'required_payload_fields': [],
  'scope_type': 'lifespan'},
 {'allowed_framings': [],
  'binding': 'lifespan',
  'channel': 'receive',
  'direction': 'system',
  'event': 'lifespan.shutdown',
  'exchange': 'unary',
  'family': 'lifespan',
  'required_payload_fields': [],
  'scope_type': 'lifespan'},
 {'allowed_framings': [],
  'binding': 'lifespan',
  'channel': 'send',
  'direction': 'system',
  'event': 'lifespan.startup.complete',
  'exchange': 'unary',
  'family': 'lifespan',
  'required_payload_fields': [],
  'scope_type': 'lifespan'},
 {'allowed_framings': [],
  'binding': 'lifespan',
  'channel': 'send',
  'direction': 'system',
  'event': 'lifespan.startup.failed',
  'exchange': 'unary',
  'family': 'lifespan',
  'required_payload_fields': ['message'],
  'scope_type': 'lifespan'},
 {'allowed_framings': [],
  'binding': 'lifespan',
  'channel': 'send',
  'direction': 'system',
  'event': 'lifespan.shutdown.complete',
  'exchange': 'unary',
  'family': 'lifespan',
  'required_payload_fields': [],
  'scope_type': 'lifespan'},
 {'allowed_framings': [],
  'binding': 'lifespan',
  'channel': 'send',
  'direction': 'system',
  'event': 'lifespan.shutdown.failed',
  'exchange': 'unary',
  'family': 'lifespan',
  'required_payload_fields': ['message'],
  'scope_type': 'lifespan'},
 {'allowed_framings': [],
  'binding': 'webtransport',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'transport.emit.complete',
  'exchange': 'duplex',
  'family': 'datagram',
  'required_payload_fields': [],
  'scope_type': 'webtransport'},
 {'allowed_framings': [],
  'binding': 'webtransport',
  'channel': 'send',
  'direction': 'server_to_client',
  'event': 'transport.emit.failed',
  'exchange': 'duplex',
  'family': 'datagram',
  'required_payload_fields': ['message'],
  'scope_type': 'webtransport'}]

SEMANTIC_DOMAINS = {'backpressure': {'capabilities': ['can_measure_send_queue',
                                   'can_apply_flow_control',
                                   'can_resume_after_drain'],
                  'initial': 'writable',
                  'owner': 'tigr-asgi-contract',
                  'states': {'congested': 'transport is above its soft '
                                          'pressure threshold',
                             'draining': 'transport is rejecting or delaying '
                                         'new work while queued work clears',
                             'resumed': 'transport returned below the resume '
                                        'threshold',
                             'saturated': 'transport is at or beyond its hard '
                                          'pressure threshold',
                             'writable': 'transport can accept more outbound '
                                         'work'},
                  'terminal': [],
                  'transitions': [{'event': 'backpressure.congested',
                                   'from': 'writable',
                                   'to': 'congested'},
                                  {'event': 'backpressure.saturated',
                                   'from': 'congested',
                                   'to': 'saturated'},
                                  {'event': 'backpressure.draining',
                                   'from': 'saturated',
                                   'to': 'draining'},
                                  {'event': 'backpressure.resumed',
                                   'from': 'draining',
                                   'to': 'resumed'},
                                  {'event': 'backpressure.writable',
                                   'from': 'resumed',
                                   'to': 'writable'},
                                  {'event': 'backpressure.resumed',
                                   'from': 'congested',
                                   'to': 'resumed'}]},
 'cancellation': {'capabilities': ['can_signal_transport_cancellation',
                                   'can_signal_application_cancellation',
                                   'can_bound_cancellation_cleanup'],
                  'initial': 'requested',
                  'owner': 'tigr-asgi-contract',
                  'states': {'acknowledged': 'responsible boundary '
                                             'acknowledged the cancellation '
                                             'request',
                             'completed': 'cancellation finished and cleanup '
                                          'ran',
                             'failed': 'cancellation delivery or cleanup '
                                       'failed',
                             'ignored': 'cancellation was explicitly ignored '
                                        'by policy',
                             'propagated': 'cancellation was delivered to the '
                                           'responsible execution boundary',
                             'requested': 'cancellation was requested by a '
                                          'transport, runtime, or application '
                                          'cause'},
                  'terminal': ['completed', 'ignored', 'failed'],
                  'transitions': [{'event': 'cancellation.propagated',
                                   'from': 'requested',
                                   'to': 'propagated'},
                                  {'event': 'cancellation.acknowledged',
                                   'from': 'propagated',
                                   'to': 'acknowledged'},
                                  {'event': 'cancellation.completed',
                                   'from': 'acknowledged',
                                   'to': 'completed'},
                                  {'event': 'cancellation.ignored',
                                   'from': 'requested',
                                   'to': 'ignored'},
                                  {'event': 'cancellation.failed',
                                   'from': 'propagated',
                                   'to': 'failed'},
                                  {'event': 'cancellation.failed',
                                   'from': 'acknowledged',
                                   'to': 'failed'}]},
 'channel_lifecycle': {'capabilities': ['can_report_channel_open',
                                        'can_report_read_closed',
                                        'can_report_write_closed',
                                        'can_report_channel_closing',
                                        'can_report_channel_loss'],
                       'capability_effects': {'closed': {'can_drain': False,
                                                         'can_read': False,
                                                         'can_write': False,
                                                         'terminal': True},
                                              'closing': {'can_drain': True,
                                                          'can_read': False,
                                                          'can_write': False,
                                                          'terminal': False},
                                              'failed': {'can_drain': False,
                                                         'can_read': False,
                                                         'can_write': False,
                                                         'terminal': True},
                                              'initialized': {'can_drain': False,
                                                              'can_read': False,
                                                              'can_write': False,
                                                              'terminal': False},
                                              'lost': {'can_drain': False,
                                                       'can_read': False,
                                                       'can_write': False,
                                                       'terminal': True},
                                              'open': {'can_drain': False,
                                                       'can_read': True,
                                                       'can_write': True,
                                                       'terminal': False},
                                              'opening': {'can_drain': False,
                                                          'can_read': False,
                                                          'can_write': False,
                                                          'terminal': False},
                                              'read_closed': {'can_drain': True,
                                                              'can_read': False,
                                                              'can_write': True,
                                                              'terminal': False},
                                              'write_closed': {'can_drain': True,
                                                               'can_read': True,
                                                               'can_write': False,
                                                               'terminal': False}},
                       'compatibility': [{'rule': 'inbound input is complete '
                                                  'and application work may '
                                                  'continue when enough input '
                                                  'exists',
                                          'state': 'read_closed'},
                                         {'rule': 'outbound completion cannot '
                                                  'advance to new transport '
                                                  'writes',
                                          'state': 'write_closed'},
                                         {'rule': 'new outbound sends are '
                                                  'suppressed while drain '
                                                  'policy is evaluated',
                                          'state': 'closing'},
                                         {'rule': 'may imply disconnect and '
                                                  'cancellation depending on '
                                                  'source',
                                          'state': 'failed'},
                                         {'rule': 'implies disconnect and may '
                                                  'request cancellation',
                                          'state': 'lost'}],
                       'initial': 'initialized',
                       'owner': 'tigr-asgi-contract',
                       'protocol_projection': [{'direction': 'read',
                                                'event': 'channel.read_closed',
                                                'protocol': 'http2',
                                                'rfc': 'RFC 9113',
                                                'section': '5.1',
                                                'state': 'read_closed',
                                                'term': 'half-closed(remote)'},
                                               {'direction': 'write',
                                                'event': 'channel.write_closed',
                                                'protocol': 'http2',
                                                'rfc': 'RFC 9113',
                                                'section': '5.1',
                                                'state': 'write_closed',
                                                'term': 'half-closed(local)'},
                                               {'direction': 'bidirectional',
                                                'event': 'channel.lost',
                                                'protocol': 'http2',
                                                'rfc': 'RFC 9113',
                                                'section': '6.4',
                                                'state': 'lost',
                                                'term': 'RST_STREAM'},
                                               {'direction': 'connection',
                                                'event': 'channel.closing',
                                                'protocol': 'http2',
                                                'rfc': 'RFC 9113',
                                                'section': '6.8',
                                                'state': 'closing',
                                                'term': 'GOAWAY'},
                                               {'direction': 'read',
                                                'event': 'channel.read_closed',
                                                'protocol': 'http3',
                                                'rfc': 'RFC 9114',
                                                'section': '4.1',
                                                'state': 'read_closed',
                                                'term': 'request stream '
                                                        'receive closed'},
                                               {'direction': 'write',
                                                'event': 'channel.write_closed',
                                                'protocol': 'http3',
                                                'rfc': 'RFC 9114',
                                                'section': '4.1',
                                                'state': 'write_closed',
                                                'term': 'request stream send '
                                                        'closed'},
                                               {'direction': 'bidirectional',
                                                'event': 'channel.lost',
                                                'protocol': 'http3',
                                                'rfc': 'RFC 9114',
                                                'section': '4.1.1',
                                                'state': 'lost',
                                                'term': 'request cancellation'},
                                               {'direction': 'connection',
                                                'event': 'channel.closing',
                                                'protocol': 'http3',
                                                'rfc': 'RFC 9114',
                                                'section': '5.2',
                                                'state': 'closing',
                                                'term': 'GOAWAY'},
                                               {'direction': 'read',
                                                'event': 'channel.read_closed',
                                                'protocol': 'quic',
                                                'rfc': 'RFC 9000',
                                                'section': '3',
                                                'state': 'read_closed',
                                                'term': 'receive stream '
                                                        'closed'},
                                               {'direction': 'write',
                                                'event': 'channel.write_closed',
                                                'protocol': 'quic',
                                                'rfc': 'RFC 9000',
                                                'section': '3',
                                                'state': 'write_closed',
                                                'term': 'send stream closed'},
                                               {'direction': 'write',
                                                'event': 'channel.write_closed',
                                                'protocol': 'quic',
                                                'rfc': 'RFC 9000',
                                                'section': '19.4',
                                                'state': 'write_closed',
                                                'term': 'RESET_STREAM'},
                                               {'direction': 'write',
                                                'event': 'channel.write_closed',
                                                'protocol': 'quic',
                                                'rfc': 'RFC 9000',
                                                'section': '19.5',
                                                'state': 'write_closed',
                                                'term': 'STOP_SENDING'},
                                               {'direction': 'connection',
                                                'event': 'channel.closing',
                                                'protocol': 'quic',
                                                'rfc': 'RFC 9000',
                                                'section': '10',
                                                'state': 'closing',
                                                'term': 'connection close'},
                                               {'direction': 'bidirectional',
                                                'event': 'channel.closing',
                                                'protocol': 'websocket_http2',
                                                'rfc': 'RFC 8441',
                                                'section': 'extended-connect',
                                                'state': 'closing',
                                                'term': 'tunnel stream ended'},
                                               {'direction': 'bidirectional',
                                                'event': 'channel.closing',
                                                'protocol': 'websocket_http3',
                                                'rfc': 'RFC 9220',
                                                'section': 'extended-connect',
                                                'state': 'closing',
                                                'term': 'tunnel stream ended'}],
                       'rfc_targets': {'http2': {'rfc': 'RFC 9113',
                                                 'sections': ['5.1',
                                                              '5.2',
                                                              '6.4',
                                                              '6.8']},
                                       'http3': {'rfc': 'RFC 9114',
                                                 'sections': ['4.1',
                                                              '4.1.1',
                                                              '5.2',
                                                              '6.1']},
                                       'quic': {'rfc': 'RFC 9000',
                                                'sections': ['3',
                                                             '3.5',
                                                             '10',
                                                             '19.4',
                                                             '19.5']},
                                       'websocket_http2': {'applies_when': 'WebSocket '
                                                                           'tunnel '
                                                                           'lifecycle '
                                                                           'is '
                                                                           'projected '
                                                                           'over '
                                                                           'an '
                                                                           'HTTP/2 '
                                                                           'stream',
                                                           'rfc': 'RFC 8441'},
                                       'websocket_http3': {'applies_when': 'WebSocket '
                                                                           'tunnel '
                                                                           'lifecycle '
                                                                           'is '
                                                                           'projected '
                                                                           'over '
                                                                           'an '
                                                                           'HTTP/3 '
                                                                           'stream',
                                                           'rfc': 'RFC 9220'}},
                       'states': {'closed': 'channel closed cleanly and no '
                                            'direction remains usable',
                                  'closing': 'channel close handshake or drain '
                                             'is in progress',
                                  'failed': 'channel failed because protocol '
                                            'or stream semantics were violated',
                                  'initialized': 'channel identity exists but '
                                                 'opening has not started',
                                  'lost': 'channel was lost without a clean '
                                          'protocol close',
                                  'open': 'channel can read and write '
                                          'according to protocol '
                                          'directionality',
                                  'opening': 'channel open negotiation is in '
                                             'progress',
                                  'read_closed': 'inbound/read side is closed; '
                                                 'outbound/write may remain '
                                                 'usable',
                                  'write_closed': 'outbound/write side is '
                                                  'closed; inbound/read may '
                                                  'remain usable'},
                       'terminal': ['closed', 'failed', 'lost'],
                       'transitions': [{'event': 'channel.opening',
                                        'from': 'initialized',
                                        'to': 'opening'},
                                       {'event': 'channel.opened',
                                        'from': 'opening',
                                        'to': 'open'},
                                       {'event': 'channel.read_closed',
                                        'from': 'open',
                                        'to': 'read_closed'},
                                       {'event': 'channel.write_closed',
                                        'from': 'open',
                                        'to': 'write_closed'},
                                       {'event': 'channel.closing',
                                        'from': 'open',
                                        'to': 'closing'},
                                       {'event': 'channel.closed',
                                        'from': 'open',
                                        'to': 'closed'},
                                       {'event': 'channel.failed',
                                        'from': 'open',
                                        'to': 'failed'},
                                       {'event': 'channel.lost',
                                        'from': 'open',
                                        'to': 'lost'},
                                       {'event': 'channel.write_closed',
                                        'from': 'read_closed',
                                        'to': 'closed'},
                                       {'event': 'channel.closing',
                                        'from': 'read_closed',
                                        'to': 'closing'},
                                       {'event': 'channel.failed',
                                        'from': 'read_closed',
                                        'to': 'failed'},
                                       {'event': 'channel.lost',
                                        'from': 'read_closed',
                                        'to': 'lost'},
                                       {'event': 'channel.read_closed',
                                        'from': 'write_closed',
                                        'to': 'closed'},
                                       {'event': 'channel.closing',
                                        'from': 'write_closed',
                                        'to': 'closing'},
                                       {'event': 'channel.failed',
                                        'from': 'write_closed',
                                        'to': 'failed'},
                                       {'event': 'channel.lost',
                                        'from': 'write_closed',
                                        'to': 'lost'},
                                       {'event': 'channel.closed',
                                        'from': 'closing',
                                        'to': 'closed'},
                                       {'event': 'channel.failed',
                                        'from': 'closing',
                                        'to': 'failed'},
                                       {'event': 'channel.lost',
                                        'from': 'closing',
                                        'to': 'lost'}]},
 'completion': {'capabilities': ['can_report_emit_acceptance',
                                 'can_report_transport_flush',
                                 'can_report_peer_acknowledgement'],
                'initial': 'accepted_by_runtime',
                'owner': 'tigr-asgi-contract',
                'states': {'aborted_by_peer': 'peer aborted the unit before '
                                              'completion was observable',
                           'accepted_by_runtime': 'runtime accepted the send '
                                                  'request and may queue it',
                           'failed_during_emit': 'send failed before the '
                                                 'requested completion level',
                           'flushed_to_transport': 'bytes or frames were '
                                                   'handed to the transport '
                                                   'boundary',
                           'peer_acknowledged': 'peer acknowledgement was '
                                                'observed where supported',
                           'queued_for_transport': 'send is queued for '
                                                   'protocol transport work'},
                'terminal': ['peer_acknowledged',
                             'failed_during_emit',
                             'aborted_by_peer'],
                'transitions': [{'event': 'completion.queued',
                                 'from': 'accepted_by_runtime',
                                 'to': 'queued_for_transport'},
                                {'event': 'completion.flushed',
                                 'from': 'queued_for_transport',
                                 'to': 'flushed_to_transport'},
                                {'event': 'completion.peer_acknowledged',
                                 'from': 'flushed_to_transport',
                                 'to': 'peer_acknowledged'},
                                {'event': 'completion.failed',
                                 'from': 'accepted_by_runtime',
                                 'to': 'failed_during_emit'},
                                {'event': 'completion.failed',
                                 'from': 'queued_for_transport',
                                 'to': 'failed_during_emit'},
                                {'event': 'completion.failed',
                                 'from': 'flushed_to_transport',
                                 'to': 'failed_during_emit'},
                                {'event': 'completion.peer_aborted',
                                 'from': 'accepted_by_runtime',
                                 'to': 'aborted_by_peer'},
                                {'event': 'completion.peer_aborted',
                                 'from': 'queued_for_transport',
                                 'to': 'aborted_by_peer'},
                                {'event': 'completion.peer_aborted',
                                 'from': 'flushed_to_transport',
                                 'to': 'aborted_by_peer'}]},
 'disconnect': {'capabilities': ['can_detect_peer_close',
                                 'can_detect_stream_reset',
                                 'can_detect_transport_loss',
                                 'can_detect_server_shutdown'],
                'initial': 'graceful',
                'owner': 'tigr-asgi-contract',
                'states': {'graceful': 'connection or stream closed cleanly',
                           'peer_reset': 'peer reset the stream, session, or '
                                         'connection',
                           'protocol_error': 'protocol violation forced '
                                             'closure',
                           'server_shutdown': 'server shutdown or worker '
                                              'draining closed the transport',
                           'timeout': 'inactivity or operation deadline closed '
                                      'the transport',
                           'transport_lost': 'transport failed without a clean '
                                             'close signal'},
                'terminal': ['graceful',
                             'peer_reset',
                             'transport_lost',
                             'timeout',
                             'protocol_error',
                             'server_shutdown'],
                'transitions': [{'event': 'disconnect.peer_reset',
                                 'from': 'graceful',
                                 'to': 'peer_reset'},
                                {'event': 'disconnect.transport_lost',
                                 'from': 'graceful',
                                 'to': 'transport_lost'},
                                {'event': 'disconnect.timeout',
                                 'from': 'graceful',
                                 'to': 'timeout'},
                                {'event': 'disconnect.protocol_error',
                                 'from': 'graceful',
                                 'to': 'protocol_error'},
                                {'event': 'disconnect.server_shutdown',
                                 'from': 'graceful',
                                 'to': 'server_shutdown'}]},
 'stream_resume': {'capabilities': ['can_validate_resume_token',
                                    'can_bind_resume_identity',
                                    'can_replay_buffered_units',
                                    'can_reject_stale_resume'],
                   'initial': 'idle',
                   'owner': 'tigr-asgi-contract',
                   'rfc_targets': {'http1': {'applies_when': 'application '
                                                             'retry or range '
                                                             'semantics are '
                                                             'projected over '
                                                             'HTTP/1.1',
                                             'rfc': 'RFC 9112'},
                                   'http2': {'applies_when': 'application '
                                                             'resume is '
                                                             'projected above '
                                                             'connection-local '
                                                             'HTTP/2 stream '
                                                             'ids',
                                             'rfc': 'RFC 9113'},
                                   'http3': {'applies_when': 'application '
                                                             'resume is '
                                                             'projected above '
                                                             'HTTP/3 request '
                                                             'stream loss',
                                             'rfc': 'RFC 9114'},
                                   'quic': {'applies_when': 'application '
                                                            'resume is '
                                                            'distinct from '
                                                            'QUIC connection '
                                                            'resumption and '
                                                            'stream ids',
                                            'rfc': 'RFC 9000'},
                                   'websocket': {'applies_when': 'application '
                                                                 'message '
                                                                 'replay is '
                                                                 'projected '
                                                                 'above a new '
                                                                 'WebSocket '
                                                                 'connection',
                                                 'rfc': 'RFC 6455'}},
                   'states': {'accepted': 'the runtime accepted the resume '
                                          'token and identity',
                              'expired': 'resume state existed but aged out '
                                         'before it could be accepted',
                              'idle': 'no resume attempt is active for the '
                                      'stream identity',
                              'rejected': 'resume was denied by policy, '
                                          'identity mismatch, or unavailable '
                                          'state',
                              'replaying': 'buffered units are being replayed '
                                           'to restore stream position',
                              'requested': 'a client or peer requested resume '
                                           'for a prior stream identity',
                              'resumed': 'resume completed and the stream may '
                                         'continue from the accepted offset'},
                   'terminal': ['resumed', 'rejected', 'expired'],
                   'transitions': [{'event': 'stream_resume.requested',
                                    'from': 'idle',
                                    'to': 'requested'},
                                   {'event': 'stream_resume.accepted',
                                    'from': 'requested',
                                    'to': 'accepted'},
                                   {'event': 'stream_resume.rejected',
                                    'from': 'requested',
                                    'to': 'rejected'},
                                   {'event': 'stream_resume.expired',
                                    'from': 'requested',
                                    'to': 'expired'},
                                   {'event': 'stream_resume.replaying',
                                    'from': 'accepted',
                                    'to': 'replaying'},
                                   {'event': 'stream_resume.completed',
                                    'from': 'accepted',
                                    'to': 'resumed'},
                                   {'event': 'stream_resume.completed',
                                    'from': 'replaying',
                                    'to': 'resumed'},
                                   {'event': 'stream_resume.rejected',
                                    'from': 'replaying',
                                    'to': 'rejected'}]}}
